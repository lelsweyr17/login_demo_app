...
class HomeTabBarView extends StatelessWidget {
  ...
  Widget? checkLoadingDataState(context, state) {
  ...
  List<Widget> tabBarViewChildren(context, state) {
    List<Widget> widgets = [];
    Widget? check = checkLoadingDataState(context, state);

    for (int i = 0; i < 4; i++) {
      widgets.add(example(context, state, i, check));
    }

    return widgets;
  }

  Widget example(context, state, num, check) {
    return (check != null)
        ? check
        : ListView.builder(
